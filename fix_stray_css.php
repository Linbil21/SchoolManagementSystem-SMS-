<?php
/**
 * FORCE OVERWRITE - Replaces server's New-Applications.php with clean version
 * DELETE AFTER USE!
 */
header('Content-Type: text/plain; charset=utf-8');

echo "=== FORCE OVERWRITE: New-Applications.php ===\n\n";
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Poppins', sans-serif; }
        ...
        .empty-icon { width: 100px; height: 100px; background: #f8fafc; border-radius: 30px; display: flex; align-items: center; justify-content: center; margin: 0 auto 20px; }
        .empty-icon i { color: #e2e8f0; font-size: 3rem; }
        .empty-text { color: #64748b; font-weight: 600; }

        /* Modal */
        .student-modal-overlay { position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(15,23,42,0.7); backdrop-filter: blur(12px); z-index: 99999; display: none; align-items: center; justify-content: center; padding: 20px; }
        .student-modal-content { background: white; width: 100%; max-width: 600px; border-radius: 35px; box-shadow: 0 30px 60px -12px rgba(0,0,0,0.3); overflow: hidden; animation: modalScale 0.4s cubic-bezier(0.34,1.56,0.64,1); }
        @keyframes modalScale { from { transform: scale(0.95); opacity: 0; } to { transform: scale(1); opacity: 1; } }

        .modal-profile-header { padding: 40px; background: linear-gradient(135deg, #1648bc 0%, #1e3a8a 100%); color: white; text-align: center; }
        .modal-avatar { width: 100px; height: 100px; border-radius: 30px; background: rgba(255,255,255,0.15); display: flex; align-items: center; justify-content: center; font-size: 2.5rem; font-weight: 800; margin: 0 auto 20px; border: 3px solid rgba(255,255,255,0.2); backdrop-filter: blur(10px); }
        .modal-name { font-weight: 800; font-size: 1.6rem; letter-spacing: -0.02em; margin-bottom: 5px; }
        .modal-course { opacity: 0.8; font-weight: 500; font-size: 0.95rem; }
        ...
        .warning-box { background: var(--warning-bg); border: 1px solid var(--warning-border); padding: 20px; border-radius: 20px; color: var(--warning-text); font-size: 0.85rem; display: flex; gap: 12px; align-items: center; }
        .warning-box p { font-weight: 600; }

        .modal-footer { padding: 30px 40px; background: white; border-top: 1px solid var(--border-soft); display: flex; justify-content: flex-end; gap: 15px; }
...
if (file_put_contents($targetFile, $cleanContent)) {
    echo "✅ SUCCESS: New-Applications.php has been overwritten with clean version!\n";
    echo "   Size: " . strlen($cleanContent) . " bytes\n\n";
} else {
...
if (strpos($written, 'student-modal-footer') !== false) {
    echo "⚠️ WARNING: stray text still present!\n";
} else {
    echo "✅ VERIFIED: No stray CSS text in file.\n";
}
...
echo "\n=== DONE! Ctrl+Shift+R to reload. DELETE this file (fix_stray_css.php) now! ===\n";
?>
